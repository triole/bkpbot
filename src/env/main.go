...
// Env holds environment information relevant for building etc.
type Env struct {
	Name        string
	MainVersion string
	Subversion  string
...
func MakeInfoString(env Env, buildtags map[string]string) (s string) {
	var sorter []string
...
func makeInfo(name string, mversion string, desc string) Env {
	return Env{
		Name:        name,
